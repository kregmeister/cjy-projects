...
class ManageMetadata:
    ...
    def get_metadata(self):
        # Retreives dataframe of ticker metadata
    ...
    def update_metadata(self, df):
        with DuckDB(self.base_path + "/sql/prices.duck") as db:
            ...
            self.update_etf_metadata(db)

    def update_etf_metadata(self, db):
    ...
    def initialize_metadata(self, df):
        with DuckDB(self.base_path + "/sql/prices.duck") as db:
            ...
            # Corrects strange behavior where statementLastUpdated is re-cast as a VARCHAR, NOT DATE on update/insert
            db.sql('''
