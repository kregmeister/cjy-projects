...
class TiingoAPI:
    ...
    def __enter__(self):
        ...
        # Acquires API key from AWS Secrets Manager
        api_key = get_credentials(["tiingo_api_key"])
    ...
    def make_request(self, url, date_columns: list = [], round=True):
        """
    ...
    def daily_metadata(self):
        ...
        df = self.make_request(url, round=False)
    ...
    def fundamentals_meta(self):
        ...
        profile_df = self.make_request(profile_url, ["statementLastUpdated", "dailyLastUpdated"], round=False)
    ...
    def search_utility(self, ticker: str):
        """
        ...
        search_url =  (
            ...
            f"query={ticker}&exactTickerMatch=true&includeDelisted=true&"
            "columns=ticker,permaTicker,isActive"
        )

        call = self.session.get(
            search_url, headers=self.headers, timeout=self.timeout
        ).json()
        # Only want delisted tickers
        resp = [item for item in call if item["isActive"] == False and item["permaTicker"].startswith("US")]
        return resp[num-2]["permaTicker"]
