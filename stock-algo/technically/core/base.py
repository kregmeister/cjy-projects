...
class Foundation:
    ...
    def execute(self):
        ...
        # Separates dev from prod environemnts (only metadata operations differ between the two)
        if self.home_dir != "/home/craig99":
            metadata.ManageMetadata(self.base_path, self.init, self.friday, dev=True).execute()
        else:
